"""SBS-enhanced Lean LSP MCP Server.

This server preserves all 18 Lean tools from upstream lean-lsp-mcp
and adds SBS-specific tools for blueprint manifest querying.
"""
...
from sbs_lsp_mcp.client_utils import (
    infer_project_path,
...
from sbs_lsp_mcp.outline_utils import generate_outline_data
from sbs_lsp_mcp.search_utils import check_ripgrep_status, lean_local_search
from sbs_lsp_mcp.utils import (
    COMPLETION_KIND,
    LeanToolError,
...
async def leansearch(
    ...
    """
    headers = {"User-Agent": "sbs-lsp-mcp/0.1", "Content-Type": "application/json"}
...
async def state_search(
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "sbs-lsp-mcp/0.1"},
        method="GET",
    )
...
async def profile_proof(
    ...
    from sbs_lsp_mcp.profile_utils import profile_theorem
