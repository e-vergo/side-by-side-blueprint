...
This server preserves all 18 Lean tools from upstream lean-lsp-mcp
and adds SLS orchestration tools for project management.
"""
...
from sls_mcp.client_utils import (
    infer_project_path,
    setup_client_for_file,
    startup_client,
)
from sls_mcp.duckdb_layer import DuckDBLayer
from sls_mcp.file_utils import get_file_contents
from sls_mcp.instructions import INSTRUCTIONS
from sls_mcp.sls_tools import register_sls_tools
from sls_mcp.skill_tools import register_skill_tools
from sls_mcp.loogle import LoogleManager, loogle_remote
from sls_mcp.models import (
    AttemptResult,
    BuildResult,
    CompletionItem,
    CompletionsResult,
...
from sls_mcp.outline_utils import generate_outline_data
from sls_mcp.search_utils import check_ripgrep_status, lean_local_search
from sls_mcp.utils import (
    COMPLETION_KIND,
    LeanToolError,
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    try:
        ...
        if os.environ.get("ZULIP_ENABLED", "").lower() in ("1", "true", "yes"):
            try:
                ...
                browser_context = await browser.new_context(
                    ...
                    user_agent="sls-mcp/0.1 Zulip Browser"
                )
        ...
        duckdb_layer: DuckDBLayer | None = None
        try:
            from sls_mcp.sls_utils import SBS_ROOT, ARCHIVE_DIR
...
if os.environ.get("ZULIP_ENABLED", "").lower() in ("1", "true", "yes"):
    try:
        from sls_mcp.zulip_tools import register_zulip_tools
...
if os.environ.get("ZULIP_ENABLED", "").lower() in ("1", "true", "yes"):
    try:
        from sls_mcp.browser_tools import register_browser_tools
...
async def leansearch(
    ...
    """
    headers = {"User-Agent": "sls-mcp/0.1", "Content-Type": "application/json"}
...
async def state_search(
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "sls-mcp/0.1"},
        method="GET",
    )
...
async def profile_proof(
    ...
    from sls_mcp.profile_utils import profile_theorem
