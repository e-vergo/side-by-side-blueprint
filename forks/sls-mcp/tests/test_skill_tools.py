...
- sls_skill_status: Query current skill state
- sls_skill_start: Start a new skill session
- sls_skill_transition: Transition between phases
- sls_skill_end: End a skill session
...
from sls_mcp.sls_models import (
    SkillEndResult,
    SkillFailResult,
    SkillStartResult,
    SkillStatusResult,
...
class TestSkillStatus:
    """Tests for sls_skill_status tool."""
...
class TestSkillStart:
    """Tests for sls_skill_start tool."""
    ...
    def test_start_when_idle(self) -> None:
        ...
        with patch("sls_mcp.sls_tools._run_archive_upload") as mock_upload:
...
class TestSkillTransition:
    """Tests for sls_skill_transition tool."""
...
class TestPhaseOrdering:
    """Tests for phase ordering enforcement in sls_skill_transition."""
...
class TestArchiveUploadHelper:
    ...
    def test_upload_builds_correct_command(self) -> None:
        ...
        with patch("sls_mcp.sls_tools.subprocess.run") as mock_run:
            ...
            from sls_mcp.sls_tools import _run_archive_upload
...
class TestImprovementCapture:
    """Tests for sls_improvement_capture tool."""
    ...
    def _simulate_capture(
        ...
        """Simulate sls_improvement_capture logic and return result.

        Mirrors the implementation in sls_tools.py without calling through MCP.
        The tool now uses db.get_global_state() instead of load_archive_index().
        """
        from sls_mcp.sls_models import ImprovementCaptureResult
