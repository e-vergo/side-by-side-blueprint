...
**On spawn:** Call `sls_archive_state()` to understand your operating context. Trust context provided by the orchestrator, but verify via MCP when uncertain.
...
**Recovery:** If spawned without explicit phase context, call `sls_archive_state()` to determine the current global state before proceeding.
...
1. Check `sls_archive_state()` for current skill/substate
2. Check `sls_skill_status()` for active skill conflicts
3. Check `sls_serve_project(action="status")` for server state
4. Use `Bash` to check build caches, file existence, git status

**Examples of explore-before-ask:**
- "Is there a running server?" → Call `sls_serve_project(project="SBSTest", action="status")` first
- "What phase are we in?" → Call `sls_archive_state()` first
...
The sls-mcp server provides SLS-specific tools for orchestration and testing:

**Orchestration:**
| Tool | Use For |
|------|---------|
| `sls_archive_state` | Check current global state and skill substate |
| `sls_context` | Build context for agent spawn |
| `sls_epoch_summary` | Get aggregated epoch data |
| `sls_search_entries` | Search archive entries by tag, project, or trigger |

**Testing & Validation:**
| Tool | Use For |
|------|---------|
| `sls_run_tests` | Run pytest suite with optional filter |
| `sls_validate_project` | Run T1-T8 validators on a project |

**Build & Visual:**
| Tool | Use For |
|------|---------|
| `sls_build_project` | Trigger full project build |
| `sls_serve_project` | Start/stop/check dev server |
| `sls_last_screenshot` | Get most recent screenshot for a page |
| `sls_visual_history` | View screenshot history across entries |
...
- When testing MCP repo tools: `cd forks/sls-mcp && .venv/bin/pytest tests/ -v`
- Use `sls_run_tests(repo="mcp")` to run MCP repo tests through the standard MCP tool
...
When encountering clear bugs during implementation, log them immediately via `sls_issue_log` MCP tool:
