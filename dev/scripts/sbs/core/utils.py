...
# Known repos in the workspace with their relative paths from SBS_ROOT
REPO_PATHS = {
...
# Backwards compatibility: list of repo names
...
def get_repo_path(name: str) -> Optional[Path]:
    """Get the full path for a repo by name.

    Returns None if repo not found in REPO_PATHS or doesn't exist.
    """
    if name not in REPO_PATHS:
        return None
    path = get_sbs_root() / REPO_PATHS[name]
