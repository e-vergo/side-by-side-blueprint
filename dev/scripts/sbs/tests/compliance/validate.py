...
def run_compliance_check(config: ComplianceConfig) -> tuple[ComplianceLedger, dict[str, float]]:
    ...
    Returns:
        Tuple of (ledger, qa_timings) where qa_timings contains per-operation timing
    """
    log.header("Visual Compliance Check")

    qa_timings: dict[str, float] = {}
    ...
    qa_timings["ledger_load"] = round(time.time() - ledger_start, 3)
    ...
    qa_timings["compute_pages"] = round(time.time() - compute_start, 3)
    ...
    save_ledger(ledger)
    qa_timings["ledger_save"] = round(time.time() - save_start, 3)

    # Total compliance check timing
    qa_timings["compliance_check_total"] = round(time.time() - check_start, 3)

    return ledger, qa_timings
...
def cmd_compliance(args) -> int:
    ...
    try:
        ...
        # Run compliance check (now returns timing data)
        ledger, qa_timings = run_compliance_check(config)
        ...
        if getattr(args, 'verbose', False) and qa_timings:
            print("\n## QA Timing")
            for key, value in qa_timings.items():
