...
|-----------|-----------|-----------|
| `forks/sls-mcp/` (was `sbs-lsp-mcp`) | 18 Lean LSP tools (now in `lean-lsp-mcp`), ~10 build/quality tools | ~33 orchestration/analysis tools, 8 skill tools, 5 browser tools, 3 Zulip tools, 1 improvement capture tool |
...
Total tools (was `sbs-lsp-mcp`, now split into `lean-lsp-mcp` + `sls-mcp`): **75** (18 Lean + 41 sls_tools + 8 skill_tools + 5 browser + 3 Zulip)
...
| Tool | Source File | Purpose |
|------|------------|---------|
| `sls_build_project` | `sls_tools.py` | Trigger full project build |
| `sls_validate_project` | `sls_tools.py` | Run T1-T8 validators on project |
| `sls_serve_project` | `sls_tools.py` | Start/stop/check dev server |
| `sls_inspect_project` | `sls_tools.py` | Prepare context for visual QA |
| `sls_run_tests` | `sls_tools.py` | Run pytest suite (shared -- tests both SBS and SLS) |
| `sls_last_screenshot` | `sls_tools.py` | Get most recent screenshot for page |
| `sls_visual_history` | `sls_tools.py` | View screenshot history across entries |
| `ask_oracle` | `sls_tools.py` | Unified query: concept index + archive + quality metrics (shared) |
...
| Tool | Source File | Purpose |
|------|------------|---------|
| `sls_archive_state` | `sls_tools.py` | Current global state and skill substate |
| `sls_epoch_summary` | `sls_tools.py` | Aggregated epoch data |
| `sls_context` | `sls_tools.py` | Build context for agent spawn |
| `sls_search_entries` | `sls_tools.py` | Search archive entries by tag/project/trigger |

**Skills (8 tools):**

| Tool | Source File | Purpose |
|------|------------|---------|
| `sls_task` | `skill_tools.py` | General-purpose task execution with validation |
| `sls_log` | `skill_tools.py` | Quick issue/idea capture to GitHub |
| `sls_qa` | `skill_tools.py` | Live interactive QA against blueprint site |
| `sls_introspect` | `skill_tools.py` | Self-improvement across hierarchy levels |
| `sls_self_improve` | `skill_tools.py` | Compute introspection level and assemble context |
| `sls_converge` | `skill_tools.py` | Autonomous QA convergence loop |
| `sls_update_and_archive` | `skill_tools.py` | Documentation refresh and porcelain state |
| `sls_divination` | `skill_tools.py` | Forecasting test outcomes and system behavior |

**Skill State Machine (6 tools):**

| Tool | Source File | Purpose |
|------|------------|---------|
| `sls_skill_status` | `sls_tools.py` | Get current skill/substate |
| `sls_skill_start` | `sls_tools.py` | Start skill, set global_state |
| `sls_skill_transition` | `sls_tools.py` | Move to next phase |
| `sls_skill_end` | `sls_tools.py` | Clear global_state |
| `sls_skill_fail` | `sls_tools.py` | Record failure and release global_state |
| `sls_skill_handoff` | `sls_tools.py` | Atomic skill-to-skill transition |

**GitHub Integration (7 tools):**

| Tool | Source File | Purpose |
|------|------------|---------|
| `sls_issue_create` | `sls_tools.py` | Create GitHub issue |
| `sls_issue_log` | `sls_tools.py` | Agent-optimized issue logging with auto-populated context |
| `sls_issue_list` | `sls_tools.py` | List issues with filters |
| `sls_issue_get` | `sls_tools.py` | Get issue details |
| `sls_issue_close` | `sls_tools.py` | Close issue |
| `sls_issue_summary` | `sls_tools.py` | Aggregate stats for open issues |
| `sls_pr_create` | `sls_tools.py` | Create pull request |
| `sls_pr_list` | `sls_tools.py` | List open PRs |
| `sls_pr_get` | `sls_tools.py` | Get PR details |
| `sls_pr_merge` | `sls_tools.py` | Merge PR to main |

**Session Analysis (14 tools):**

| Tool | Source File | Purpose |
|------|------------|---------|
| `sls_analysis_summary` | `sls_tools.py` | Aggregated analysis metrics |
| `sls_entries_since_self_improve` | `sls_tools.py` | Entries since last self-improve |
| `sls_successful_sessions` | `sls_tools.py` | Mine successful session patterns |
| `sls_comparative_analysis` | `sls_tools.py` | Compare session characteristics |
| `sls_system_health` | `sls_tools.py` | System health metrics |
| `sls_user_patterns` | `sls_tools.py` | User interaction patterns |
| `sls_skill_stats` | `sls_tools.py` | Per-skill usage statistics |
| `sls_phase_transition_health` | `sls_tools.py` | Phase transition success rates |
| `sls_interruption_analysis` | `sls_tools.py` | Session interruption patterns |
| `sls_gate_failures` | `sls_tools.py` | Gate validation failure analysis |
| `sls_tag_effectiveness` | `sls_tools.py` | Tag coverage and effectiveness |
| `sls_question_analysis` | `sls_tools.py` | AskUserQuestion interaction extraction |
| `sls_question_stats` | `sls_tools.py` | Aggregate question usage statistics |
| `sls_improvement_capture` | `sls_tools.py` | Capture improvement observations |
...
4. Remove Lean LSP tools from `sls-mcp/server.py` (was `sbs-lsp-mcp`)
5. Update `.mcp.json` to reference both `lean-lsp-mcp` and `sls-mcp`
...
**Breakage:** Agent definitions referenced `mcp__sbs-lsp__lean_*` tool names. These became `mcp__lean-lsp__lean_*`. All agent `.md` files needed tool name prefix updates (completed in Wave 3).
...
**Steps:**
1. Create `sbs-tools` package with Category B tools (8 tools from `sls_tools.py`)
2. Move `sls_models.py`, `sls_utils.py` to `sbs-tools`
3. Rename `sbs-lsp-mcp` to `sls-mcp` (completed)
...
7. Rename tool prefixes: `sbs_archive_state` -> `sls_archive_state`, etc. (completed in Wave 3)
...
**Current state:** The monorepo contains submodules for Dress, Runway, SBS-Test, dress-blueprint-action, subverso, verso, LeanArchitect, sls-mcp (was sbs-lsp-mcp), and vscode-lean4.

**After split:**
- SBS repo: Dress, Runway, SBS-Test, dress-blueprint-action, subverso, verso, LeanArchitect (move from current monorepo)
- SLS repo: sls-mcp (renamed from sbs-lsp-mcp, completed), vscode-lean4 (move from current monorepo)
