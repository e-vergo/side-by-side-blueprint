...
### sls-mcp: SLS Orchestration Server

The `sls-mcp` server (located at `forks/sls-mcp/`, renamed from `sbs-lsp-mcp`) provides SLS orchestration tools. Lean LSP tools are now served separately by `lean-lsp-mcp`.

**Design principle:** General-purpose tools, not hyper-specific agents.

**Implementation:** 49 SLS tools (41 orchestration + 5 Browser + 3 Zulip) + 18 Lean tools via `lean-lsp-mcp`. See `forks/sls-mcp/README.md` for full documentation.
...
### Added Capabilities (41 SLS-specific Tools + 5 Browser + 3 Zulip)
...
| **Archive** | `sls_archive_state`, `sls_context`, `sls_epoch_summary`, `sls_search_entries`, `sls_analysis_summary`, `sls_entries_since_self_improve` | State, context, and history |
| **Self-Improve** | `sls_successful_sessions`, `sls_comparative_analysis`, `sls_system_health`, `sls_user_patterns`, `sls_skill_stats`, `sls_phase_transition_health`, `sls_interruption_analysis`, `sls_gate_failures`, `sls_tag_effectiveness` | Structured analysis for self-improvement |
| **Testing** | `sls_run_tests`, `sls_validate_project` | Pytest and T1-T8 validators |
| **Build** | `sls_build_project`, `sls_serve_project`, `sls_last_screenshot`, `sls_visual_history`, `sls_inspect_project` | Build, serve, visual checks, and QA context |
| **GitHub** | `sls_issue_create`, `sls_issue_log`, `sls_issue_list`, `sls_issue_get`, `sls_issue_close`, `sls_issue_summary`, `sls_pr_create`, `sls_pr_list`, `sls_pr_get`, `sls_pr_merge` | Issue and PR management (including agent-optimized logging) |
| **Skills** | `sls_skill_status`, `sls_skill_start`, `sls_skill_transition`, `sls_skill_end`, `sls_skill_fail`, `sls_skill_handoff` | Skill state machine (including failure recording and atomic handoffs) |
| **Questions** | `sls_question_analysis`, `sls_question_stats` | AskUserQuestion interaction analysis |
...
```
+----------------+     +------------------+     +---------------+
| Orchestrator   | --> | sls_archive_     | --> | Context blob  |
...
**Output:** Finding documents in dev/storage/archive/self-improve/, GitHub issues via sls_issue_log
